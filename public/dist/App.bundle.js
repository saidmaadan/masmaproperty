...
function ajaxFavorite(e) {
  ...
  _axios2.default.post(this.action).then(function (res) {
    var isFavorite = _this.favorite.classList.toggle('heart__button--hearted');
    (0, _bling.$)('.heart-count').textContent = res.data.favorites.length;
    if (isFavorite) {
      _this.favorite.classList.add('heart__button--float');
      setTimeout(function () {
        return _this.favorite.classList.remove('heart__button--float');
      }, 2500);
    }
  }).catch(console.error);
}
...
function loadPlaces(map) {
  var lat = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : 30.2;
  var lng = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : -97.7;

  _axios2.default.get('/api/listings/near?lat=' + lat + '&lng=' + lng).then(function (res) {
    var places = res.data;
    if (!places.length) {
      return;
    }
...
function typeAhead(search) {
  ...
  searchInput.on('input', function () {
    ...
    _axios2.default.get('/api/search?q=' + this.value).then(function (res) {
      ...
      searchResults.innerHTML = _dompurify2.default.sanitize('<div class="search__result">No results found for ' + _this.value);
    }).catch(function (err) {
      console.error(err);
    });
  });
